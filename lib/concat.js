...
function readToC(dir) {
    try {
        return yaml.safeLoad(readFileSync(path.join(dir, 'toc.yaml'), 'utf8'));
    } catch (_) {
        return {};
    }
}
...
function concat(source, files, dirName) {
    ...
    if (tocIsEmpty) {
        glob
            ...
            .forEach((dir) => {
                ...
                content += concat(path.join(source, dir), files, newDirName).content;
            });
    } else {
