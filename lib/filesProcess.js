...
function filesProcess({
    ...
    processToc, failOnErrors, config, failOnWarns,
}) {
    ...
    const yfmConfig = config ? config : path.join(source, '.yfm');
    ...
    try {
        ...
        if (processToc) {
            throw new Error(`There are some problem's with toc.yaml: ${e}`);
        }
    }
    const results = {};
    ...
    const numberOfErrors = log.getErrorsCount();
    const numberOfWarnings = log.getWarningsCount();
    const errMessageEnd = `${numberOfErrors} ${numberOfErrors === 1 ? 'error' : 'errors'}`;

    if (failOnWarns && numberOfWarnings) {
        const warnMessageEnd = `${numberOfWarnings} ${numberOfWarnings === 1 ? 'warning' : 'warnings'}`;
        throw new Error(`Flag fail-on-warns is enabled. Build failed with ${warnMessageEnd} and ${errMessageEnd}`);
    } else if (failOnErrors && numberOfErrors) {
        throw new Error(`Flag fail-on-errors is enabled. Build failed with ${errMessageEnd}`);
    }
