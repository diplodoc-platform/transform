...
const {alerts, attrs, anchors, code, cut, deflist, imsize, meta, sup, tabs, titles} = plugins;
...
function transform(originInput, opts = {}) {
    const {
        ...
        plugins = [attrs, meta, deflist, cut, alerts, anchors, tabs, code, titles, imsize, sup],
    } = opts;

    const input = liquid(originInput, vars, path);

    const md = new MarkdownIt({html: allowHTML, linkify, highlight, breaks});
    plugins.forEach((plugin) => md.use(plugin, opts));
