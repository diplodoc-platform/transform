...
module.exports = function transform(input, opts = {}) {
    const {
        ...
        extractTitle: extractTitleOption, needTitle, notFoundCb = () => {}, oneFile, allowHTML = false
    } = opts;

    let md;

    const builtVars = buildVars(vars, varsPreset, path, root);
    input = liquid(input, builtVars, path);

    if (!plugins) {
        md = new MarkdownIt({html: allowHTML, linkify: true, highlight})
            .use(attrs)
            .use(meta)
            .use(deflist)
