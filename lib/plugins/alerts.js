            ...
            if (
                ...
                if (!match) {
                    if (wrongNotes && tokens[i + 1].content !== '{% endnote %}') {
                        ...
                        const linkToSyntax = 'https://github.yandex-team.ru/data-ui/yfm/blob/master/DOCS.md' +
                            '#%D0%B7%D0%B0%D0%BC%D0%B5%D1%82%D0%BA%D0%B8-';
                        const mismatchLine = tokens[i + 1].map[0] + 1;
                        log.warn(
                            `Incorrect syntax for notes in line ${mismatchLine}! Check out syntax: ${linkToSyntax}`,
                        );
                    }
                    i += 3;
                    continue;
