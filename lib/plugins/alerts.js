...
const ALERT_RE = /^{% alert (note|tip|warning|important) %}$/;
...
function findCloseTokenIdx(tokens, idx) {
    ...
    while (i < tokens.length) {
        if (
            ...
            tokens[i + 1].content.trim() === '{% endalert %}'
        ) {
            return i;
        }
    ...
    md.core.ruler.push('notes', (state) => {
        ...
        while (i < tokens.length) {
            if (
                ...
                newOpenToken.attrSet('class', `alert ${type}`);
                ...
                inlineText.content = getTitle(type, lang);
