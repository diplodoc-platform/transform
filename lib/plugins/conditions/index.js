...
function inlineConditions(ifTag, vars, i, openToken, match, skipError) {
    let leftPart = '';
    ...
    openToken.children.forEach((childToken) => {
        ...
        if (evalExp(ifTag.condition, vars, skipError)) {
            ...
            ifTag.elses.some(({conditions, startPos, raw}) => {
                const isTruthy = !conditions || evalExp(conditions, vars, skipError);
...
function blockConditions(ifTag, vars, i, tokens, skipError) {
    let newTokens = [];
...
function conditions(tokens, vars, skipError) {
    const tagStack = [];
    let i = 0;
            ...
            switch (type) {
                ...
                case 'endif': {
                    ...
                    if (i - 1 === ifTag.startRowIdx) {
                        R_LIQUID.lastIndex = inlineConditions(ifTag, vars, i, openToken, match, skipError);
                    } else {
                        i = blockConditions(ifTag, vars, i, tokens, skipError);
                    }
                    break;
                }
            }
...
module.exports = (md, {vars, skipError}) => (
    md.core.ruler.push('conditions', ({tokens}) => conditions(tokens, vars, skipError))
);
