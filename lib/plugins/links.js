...
const addTitle = ({hash, file, state, opts, isEmptyLink, tokens, idx, linkToken, nextToken}) => {
    const id = hash && hash.slice(1);
    const fileTokens = getFileTokens(file, state, {...opts, disableLint: true});
    ...
    if (title) {
        ...
        linkToken.attrSet('YFM002', true);
    }
};

// eslint-disable-next-line complexity
function processLink(state, tokens, idx, opts) {
    const {path: startPath, root, transformLink, notFoundCb, needSkipLinkFn} = opts;
    ...
    if ((isEmptyLink || isTitleRefLink) && fileExists && isPageFile) {
        addTitle({hash, file, state, opts, isEmptyLink, tokens, idx, linkToken, nextToken});
    }
