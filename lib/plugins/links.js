...
const {getFileTokens, findBlockTokens} = require('../utils');
...
function getTitleFromTokens(tokens) {
    ...
    while (i < tokens.length) {
        ...
        if (token.type === 'heading_open' && nextToken.type === 'inline') {
            if (nextToken.children[0].type === 'text' && nextToken.children[0].content) {
                title = nextToken.children[0].content;
            } else {
...
function createLinks(filePath, opts) {
            ...
            if (isLink && href) {
                ...
                if (isLinkCloseToken || isTitleRefToken) {
                    ...
                    if (fileNotExist) {
                        ...
                        console.error(`Couldn't resolve title "${nextToken.type === 'text' ? nextToken.content : ''}" in file: ${filePath}`);
                    } else {
                        if (nextToken.type === 'link_close') {
                            nextToken = new Token('text', '', 0);
...
module.exports = function (md, path, opts = {}) {
    md.inline.ruler2.push('links', createLinks(path, opts));
};
