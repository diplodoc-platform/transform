...
function cutSourceFragment(source, lang, markers, includePath) {
    ...
    if (markers) {
        ...
        if (startLine < 0) {
            ...
            log.warn(`Marker '${startMarker}' not found in ${includePath}, including source fragment from the beginning of the file.`);
            startLine = 0;
        }
        if (endLine < 0) {
            // eslint-disable-next-line max-len
            log.warn(`Marker '${endMarker}' not found in ${includePath}, including source fragment till the end of the file.`);
            endLine = lines.length;
        }
        if (endLine <= startLine) {
            // eslint-disable-next-line max-len
            log.warn(`End marker '${endMarker}' occurred earlier than start marker '${startMarker}' in ${includePath}, fragment will be empty.`);
        }
    } else {
        startLine = 0;
        endLine = lines.length;
...
function getSourceTokens(path, state, {lang, lines, includePath}) {
    let sourceFragment;
    ...
    if (sourceFragmentsCache[sourceFragmentKey]) {
        ...
        sourceFragment = cutSourceFragment(content, lang, lines, includePath);
