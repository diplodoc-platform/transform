...
export function callPlugin<T>(plugin: MarkdownItPluginCb<T>, tokens: Token[], opts?: Partial<MarkdownItPluginOpts & T>) {
    md.disable = () => md;
    md.enable = () => md;
    ...
    const fakeMd = {
        core: {
            ruler: {
                push: (_name: any, cb: (s: StateCore) => void) => cb(state),
                before: (_anotherPlugin: any, _name: any, cb: (s: StateCore) => void) => cb(state),
            },
        },
    } as MarkdownIt;

    plugin(fakeMd, Object.assign({
        path: '',
        log: Logger,
        lang: 'ru',
        root: '',
